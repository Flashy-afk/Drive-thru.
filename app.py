...
class CajeroVisual(threading.Thread):
    ...
    def atender_cliente(self, cliente):
        ...
        tiempo_preparacion = random.randint(2, 4)
        ...
        if (cliente.total > 0):
            ...
            time.sleep(1.2) # Pausa activa para apreciar el coche en ventanilla
...
st.write("Visualización isométrica corregida: El flujo vehicular rodea externamente la estructura del establecimiento.")
st.write("---")
...
if (st.session_state.simulacion_activa):
    ...
    # --- RENDERIZADO DEL MAPA ISOMÉTRICO CORREGIDO ---
    while (st.session_state.v1.is_alive() or st.session_state.v2.is_alive()):
        ...
        <svg width="100%" height="480" viewBox="0 0 800 480" style="background-color:#2c6b3f; border-radius:15px; font-family: sans-serif; box-shadow: inset 0 0 40px rgba(0,0,0,0.3);">
            ...
            # TEXTO DE MARCA EN PARED PRINCIPAL
        ...
        # COORDENADAS DEL CARRIL EXTERIOR (Rodeando ordenadamente por detrás)
        puntos_carril = [
            (575, 205), (570, 145), (515, 100), (425, 80), (330, 95), (245, 130), (175, 175)
        ]

        # RENDERIZAR VEHÍCULO EN VENTANILLA 1 (Frente a su módulo correspondiente)
        ...
        # RENDERIZAR VEHÍCULO EN VENTANILLA 2 (Frente a su módulo correspondiente)
        ...
        # RENDERIZAR VEHÍCULOS ESPERANDO EN COLA (No obstruyen el edificio)
        for index, auto in enumerate(st.session_state.cola_autos):
            if (index < len(puntos_carril)):
                ...
                <g transform="translate({cx}, {cy})">
                    ...
                    <polygon points="-18,0 4,-10 20,-2 0,8" fill="#1976d2"/>
                    <polygon points="-8,-4 4,-9 14,-5 2,1" fill="#42a5f5"/>
